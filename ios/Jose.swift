...
class JoseUtil: NSObject {
  ...
  static func createJwt(header: String, payload: String, jwk: SecKey) throws -> String {
    guard let header = headerStringToHeader(header) else {
      throw ExpoAtprotoAuthError.invalidHeader("could not parse header string")
    }

    guard let payload = payloadStringToPayload(payload) else {
      throw ExpoAtprotoAuthError.invalidPayload("could not parse payload string")
    }

    let signer = Signer(signingAlgorithm: .ES256, key: jwk)

    guard let signer = signer else {
      throw ExpoAtprotoAuthError.nullSigner
    }
  ...
  static func verifyJwt(token: String, jwk: SecKey, options: VerifyOptions) throws -> VerifyResponse {
    guard let jws = try? JWS(compactSerialization: token),
      ...
      throw ExpoAtprotoAuthError.invalidJwk
    }

    let header = validation.header
    let payload = String(data: validation.payload.data(), encoding: .utf8)
    guard let payload = payload else {
      throw ExpoAtprotoAuthError.invalidPayload("unable to parse payload")
    }
    ...
    if let typ = options.typ {
      if header.typ != typ {
        throw ExpoAtprotoAuthError.invalidPayload("typ mismatch")
      }
    }
    ...
    if let requiredClaims = options.requiredClaims {
      try requiredClaims.forEach { c in
        if claims?[c] == nil {
          throw ExpoAtprotoAuthError.invalidPayload("required claim \(c) missing")
        }
      }
    }
    ...
    if let audience = options.audience {
      if claims?["aud"] as? String != audience {
        throw ExpoAtprotoAuthError.invalidPayload("audience mismatch")
      }
    }

    if let subject = options.subject {
      if claims?["sub"] as? String != subject {
        throw ExpoAtprotoAuthError.invalidPayload("subject mismatch")
      }
    }
    ...
    if let checkTolerance = options.clockTolerance {
      ...
      if let expiryString = claims?["exp"] as? String {
        ...
        throw ExpoAtprotoAuthError.invalidPayload("expiry missing")
      }
      if expiryDate < now - checkTolerance {
        throw ExpoAtprotoAuthError.invalidPayload("token expired")
      }
    }
    ...
    if let issuer = options.issuer {
      if claims?["iss"] as? String != issuer {
        throw ExpoAtprotoAuthError.invalidPayload("issuer mismatch")
      }
    }
